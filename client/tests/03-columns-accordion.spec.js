...
// Coverage item 3: h/l column switch, Enter opening a project, and Enter/Space
// toggling the 演技 accordion (announcer rows are a no-op). App.svelte
...
test('Enter / Space toggle the 演技 accordion; アナ rows are inert', async ({ page }) => {
	...
	await page.keyboard.press(' '); // Space also opens
	await expect(page.locator('.expansion')).toBeVisible();
	await page.keyboard.press(' '); // and closes
	await expect(page.locator('.expansion')).toHaveCount(0);
