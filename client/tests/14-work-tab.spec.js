...
// 問いかけプールの本文一覧 (発話内容の検証用)。エントリは {eye, script}。
const poolTexts = (category) => workLines[category].map((e) => e.script[0].text);
...
test('作業タブに切り替わりタイマー盤が出る。vimショートカットは無反応', async ({ page }) => {
	...
	await page.keyboard.press('n'); // router.tab !== 'script' -> ignored
	await expect(page.locator('.megaphone')).toHaveAttribute('aria-pressed', 'false');
});

test('タブは URL と同期し、リロードや直リンクでも保たれる', async ({ page }) => {
	...
	await page.reload(); // Ctrl+R 相当。以前はここで台本タブに戻っていた
...
test('作業→休憩→「どうする?」で止まり、もう1セットはユーザーが決める', async ({ page }) => {
	...
	await expect.poll(() => said(requests)).toBe(2); // 休憩入りシーケンスの「そこまで」

	// シーケンスの 5 秒待ちは「そこまで」の再生完了後に非同期で予約されるため、
	// 乗るまで少しずつ進める (一気に飛ばすと待ちタイマーが偽クロックに乗らない)。
	await expect
		.poll(async () => {
			await page.clock.fastForward(1_000);
			return said(requests);
		})
		.toBe(3); // 目を閉じて「うぅ、疲れたぁ……」

	await page.clock.fastForward(5 * 60_000); // 休憩満了 -> 問いかけて待つ
	await expect(panel).toHaveAttribute('data-phase', 'idle');
	await expect(page.locator('.phase-label')).toHaveText('どうする?');
	await expect.poll(() => said(requests)).toBe(4); // askNext の問いかけ
	...
	await expect(panel).toHaveAttribute('data-phase', 'work');
	await expect.poll(() => said(requests)).toBe(5); // start
});
...
for (const [timeStr, pool] of [
	...
	test(`休憩明けの問いかけは時間帯で変わる (${pool})`, async ({ page }) => {
		...
		await expect.poll(() => said(requests)).toBe(2); // そこまで
		// 5 秒待ちが偽クロックに乗るまで少しずつ進める (予約は再生完了後の非同期)。
		await expect
			.poll(async () => {
				await page.clock.fastForward(1_000);
				return said(requests);
		...
		await expect.poll(() => said(requests)).toBe(4); // 問いかけ
		const ask = requests.filter((r) => r.path === '/say' && Array.isArray(r.json)).at(-1);
...
test('LLM 生成: script がそのまま /say に流れる', async ({ page }) => {
...
test('LLM 生成: /work/talk が 502 なら固定セリフにフォールバックする', async ({ page }) => {
	await page.addInitScript(() => {
