...
14. 作業タブ: タブ切替とショートカット無効、`page.clock` によるフェーズ遷移 (作業→休憩→作業)、節目・チャッターの声かけ (`/say` への script JSON POST)、一時停止/再開の期限再計算、スリープ復帰の resync 潰し、休憩中のセット数減、LLM 声かけ (`/work/talk`) の成功経路と 502 フォールバック、声かけ OFF (`14-work-tab`)
