...
14. 作業タブ: タブ切替とショートカット無効、`page.clock` によるフェーズ遷移 (作業→休憩→「どうする?」で停止→ユーザーがもう1セット)、終了時のおつかれさま、節目・チャッター (独り言) の声かけ (`/say` への script JSON POST)、一時停止/再開の期限再計算、スリープ復帰の resync 潰し、LLM 声かけ (`/work/talk`) の成功経路と 502 フォールバック、声かけ OFF・おしゃべり「なし」(`14-work-tab`)
