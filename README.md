...
- `client/` — 管理画面 SPA (別途 `bun run build:client` でビルドし `client/build` へ出力)
...
bun run build:client        # 管理画面 SPA を client/build へビルド
...
VOICEPEAK は商用ソフトのためイメージには**同梱しない**。ホスト側の VOICEPEAK
インストールディレクトリを `/opt/voicepeak` に volume mount して使う (musl では
VOICEPEAK が動かないため、ランタイムは `debian:stable-slim` ベース。alpine 不可)。

```sh
docker build -t moca-server .
```
...
services:
  moca-server:
    ...
    ports:
      - "3000:3000"
    environment:
      VOICEPEAK: /opt/voicepeak/voicepeak
      MOCA_NARRATOR: Miyamai Moca
      ANALYZE_BACKEND: none
    volumes:
      - /home/youruser/tools/Voicepeak:/opt/voicepeak:ro  # ホストの VOICEPEAK を mount
      - moca-data:/data                                   # SQLite DB を永続化

volumes:
  moca-data:
```

`DATABASE_PATH` はイメージ既定で `/data/moca.db` (VOLUME `/data`)。VOICEPEAK は
`dic/` `fonts/` `settings/` 等を同ディレクトリから参照するため、インストール先を
ディレクトリごと mount する。
